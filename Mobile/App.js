...
export default function App() {
  ...
  const gerarExcel = async () => {
    let csv = "\ufeffCliente;${dados.cliente || '---'}\nImóvel;${dados.imovel || '---'}\n\nDescrição;Qtd;Juros %;Parcela\n";
    csv += `Mensalidades;${dados.mQtd};${dados.mJuros}%;${calculos.parcelaMensal.toFixed(2)}\n`;
    if (calculos.parcelaAnual > 0) csv += `Anuais;${dados.aQtd};${dados.aJuros}%;${calculos.parcelaAnual.toFixed(2)}\n`;
    const fileUri = FileSystem.cacheDirectory + "Simulacao.csv";
    try {
      await FileSystem.writeAsStringAsync(fileUri, csv, { encoding: FileSystem.EncodingType.UTF8 });
      await Sharing.shareAsync(fileUri);
    } catch (e) { Alert.alert("Erro", "Falha ao gerar Excel"); }
  };
...
const styles = StyleSheet.create({
  ...
  res: { textAlign: 'center', fontWeight: 'bold', marginTop: 10, color: '#2c3e50' },
